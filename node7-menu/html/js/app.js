(function () {
    ...
    function node7Tone(type = 'nav') {
        try {
            const now = Date.now();
            if (now - Node7Audio.lastPlayed < 35) return;
            Node7Audio.lastPlayed = now;

            const AudioContextClass = window.AudioContext || window.webkitAudioContext;
            if (!AudioContextClass) return;

            if (!Node7Audio.context) {
                Node7Audio.context = new AudioContextClass();
            }

            const ctx = Node7Audio.context;
            const osc = ctx.createOscillator();
            const gain = ctx.createGain();
            const base = type === 'submit' || type === 'select' ? 620 : type === 'close' ? 280 : 430;

            osc.type = 'sine';
            osc.frequency.setValueAtTime(base, ctx.currentTime);
            osc.frequency.exponentialRampToValueAtTime(base * 0.72, ctx.currentTime + 0.08);
            gain.gain.setValueAtTime(0.0001, ctx.currentTime);
            gain.gain.exponentialRampToValueAtTime(0.045, ctx.currentTime + 0.012);
            gain.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + 0.095);
    ...
    function node7Sound(type = 'nav') {
        node7Tone(type);
        try {
            if (typeof MenuData !== 'undefined' && MenuData.ResourceName) {
                $.post('https://' + MenuData.ResourceName + '/playsound', JSON.stringify({ type: type }));
            }
        } catch (e) {}
    }
    ...
    const MenuTpl = `
        <div id="menu_{{_namespace}}_{{_name}}" class="menu{{#align}} align-{{align}}{{/align}}">
            ...
            {{#hasParent}}
                <div class="node7-breadcrumb">BACKSPACE / ESC · Back to {{{parentTitle}}}</div>
            {{/hasParent}}
            ...
            <div class="menu-items">
                    ...
                    <div class="grid-container">
                        {{#elements}}
                            <div class="grid-item {{#selected}}selected{{/selected}}">
                                {{#image}}
                                    <img src="nui://node7-inventory/html/img/items/{{{image}}}.png"></img>
                                {{/image}}
                                {{^image}}
                ...
                {{^isGrid}}
                    {{#elements}}
                        ...
                        {{/isNotSelectable}}
                            {{#image}}
                                <img class="item-image" src="nui://node7-inventory/html/img/items/{{{image}}}.png"></img>
                            {{/image}}
                            ...
                            {{^isLabelSlider}}
                                ...
                                {{/tickBox}}
                                {{^tickBox}}
                                    <div id="item-label" class="label-{{labelPos}} node7-item-label" {{#image}}class="image-pad"{{/image}}>
                                        <span>{{{label}}}</span>
                                        {{#hasChildren}}
                                            <span class="node7-submenu-marker">{{{rightLabel}}}</span>
                                        {{/hasChildren}}
                                    </div>
                                {{/tickBox}}
                            {{/isLabelSlider}}
    ...
    MenuData.open = function (namespace, name, data) {
        ...
        const parentFocus = MenuData.getFocused();
        data._index = MenuData.focus.length;
        data._namespace = namespace;
        data._name = name;
        data.hasParent = data._index > 0;
        data.parentTitle = '';

        if (parentFocus && MenuData.opened[parentFocus.namespace] && MenuData.opened[parentFocus.namespace][parentFocus.name]) {
            const parentMenu = MenuData.opened[parentFocus.namespace][parentFocus.name];
            data.parentTitle = parentMenu.title || parentFocus.name || 'Previous';
        }
    ...
    window.onData = (data) => {
        ...
        switch (data.ak_menubase_action) {
            ...
            case "closeAll": {
                ...
                MenuData.pos = {};
                MenuData.render();
                document.body.classList.remove('node7-menu-open');
                break;
            }
                ...
                switch (data.ak_menubase_control) {
                    ...
                    case "BACKSPACE": {
                        ...
                        if (typeof focused != "undefined") {
                            ...
                            if (MenuData.focus.length <= 1) {
                                $.post("https://" + MenuData.ResourceName + "/closeui",
                                    JSON.stringify({ namespace: focused.namespace, name: focused.name, force: true })
                                );
                            }
                        }
                        break;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.addEventListener('keydown', function (e) {
            if (window.isInputOpen || e.repeat) return;

            const focusedMenu = MenuData.getFocused();
            const menuIsOpen = focusedMenu && MenuData.opened[focusedMenu.namespace] && MenuData.opened[focusedMenu.namespace][focusedMenu.name];
            ...
            const control = controlMap[e.key] || controlMap[e.code];
            if (!control) return;
